...
export class TransactionService {
  ...
  async saveVOut(vout: any): Promise<BlockchainVOut> {
    ...
    voutData.asset = vout?.scriptPubKey?.asset?.name ?? 'EVR';
  ...
  async createTransaction({
    ...
    try {
      ...
      utxos.forEach((utxo) => {
        const assetName = utxo.asset || 'EVR';
      ...
      if (!eachOutputAsset.includes('EVR') && !utxoAssetAmounts['EVR']) {
        utxoErrorMessage = `Insufficient EVR balance to create transaction`;
      }
      if (utxoErrorMessage) {
  ...
  async generateTransaction({
    ...
    try {
      ...
      const estimatedTxSize =
        ...
        (outputArray.length + eachOutputAsset.includes('EVR')
          ? eachOutputAsset.length
          : eachOutputAsset.length + 1) *
          34 +
        10;
      const txSizeInKB = estimatedTxSize / 1024;
      console.log('txSizeInKB', txSizeInKB);
      const calculatedTransactionFees = txSizeInKB * feeRate;
      ...
      if (estimationFees !== calculatedTransactionFees) {
        ...
        const newTransaction = await evrmore.createUnsignedTx(
          inputArray,
          outputArray,
          changeSource,
          Math.ceil(feeRate / 1024) * 2
        );
        const psbt = await evrmore.Psbt.fromHex(newTransaction);
