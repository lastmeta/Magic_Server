      ...
      await Promise.all(
        derivedAddresses.map(async (derivedElement) => {
          ...
          const balance: {
            confirmed?: number;
            unconfirmed?: number;
          } = await this.getOnlyBalance({ scripthash: derivedElement.scripthash });
          const isBalanceChanged =
            Object.keys(balance).length > 0 && balance?.confirmed > 0 && balance?.unconfirmed > 0;
          console.log('balance', balance, isBalanceChanged);
